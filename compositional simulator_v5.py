...
class CompositionalSimulator1D:
    ...
    def mscf_day_to_lbmol_day(self, rate_mscf_day: float) -> float:
        # Approximate standard gas conversion using 379.3 scf/lbmol.
        return rate_mscf_day * 1000.0 / 379.3
    ...
    def injector_source(self, state: SimulationState) -> tuple[int | None, np.ndarray | None]:
        ...
        if zinj is None:
            return None, None
        source = self.mscf_day_to_lbmol_day(injector.rate_mscf_day) * zinj
    ...
    def run(self, state0: SimulationState, t_end_days: float, dt_days: float) -> Tuple[SimulationState, SimulationHistory]:
        ...
        while t <= t_end_days + 1e-12:
            ...
            if injector is None or self.scenario.name == "natural_depletion":
                ...
                history.injector_rate_total.append(float(injector.rate_mscf_day))
...
def plot_history(history: SimulationHistory, output_dir: Path) -> None:
    ...
    plt.xlabel("Time (days)")
    plt.ylabel("Injector Rate (Mscf/day)")
...
def run_example(
    ...
    injection_rate_mscf_day: float = 0.0,
    ...
    if scenario_name in ("gas_cycling", "lean_gas_injection"):
        ...
        injector = InjectorWell(cell_index=0, rate_mscf_day=injection_rate_mscf_day, injection_composition=inj_comp)
...
def display_streamlit_app() -> None:
    ...
    injection_rate_unit = st.sidebar.selectbox(
        "Injection rate unit",
        ["Mscf/day", "MMscf/day"],
        index=0,
    )
    injection_rate_input = st.sidebar.number_input(
        f"Injection rate ({injection_rate_unit})",
        min_value=0.0,
        max_value=1.0e6,
        value=5.0,
        step=0.5,
    )
    injection_rate_mscf_day = injection_rate_input if injection_rate_unit == "Mscf/day" else injection_rate_input * 1000.0
    ...
    if scenario_name != "natural_depletion" and injection_rate_mscf_day <= 0.0:
    ...
    if st.button("Run Simulation", type="primary"):
        with st.spinner("Running compositional simulation..."):
            result = run_example(
                ...
                injection_rate_mscf_day=float(injection_rate_mscf_day),
        ...
        if scenario_name != "natural_depletion":
            ...
            inj_disp = history.injector_rate_total[-1] if injection_rate_unit == "Mscf/day" else history.injector_rate_total[-1] / 1000.0
            st.write(f"Injection rate: {inj_disp:.2f} {injection_rate_unit}")
        ...
        inj_metric = history.injector_rate_total[-1] if injection_rate_unit == "Mscf/day" else history.injector_rate_total[-1] / 1000.0
        c7.metric(f"Injector Rate ({injection_rate_unit})", f"{inj_metric:.2f}")
